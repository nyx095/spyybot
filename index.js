// WELCOME TO SC YUSUFZ
// THANKS TO NYA JAN DIAPUS
// FREE APIKEY
...
lolkey = 'Beli/Gratis'
namaowner = 'YusufRmdn'
nomorown = '62895704368804'
nomorbot = '62895704368804'
blocked = []
...
async function starts() {
	...
	yusufz.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan Kode QRnya Bro'))
	})

	fs.existsSync('./SpyBot.json') && yusufz.loadAuthInfo('./SpyBot.json')
	yusufz.on('connecting', () => {
		start('2', 'LAGI NYAMBUNGIN BG....')
	})
	yusufz.on('open', () => {
		success('2', '[] TESAMBUNG')
	})
	await yusufz.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./SpyBot.json', JSON.stringify(yusufz.base64EncodedAuthInfo(), null, '\t'))
    ...
    yusufz.on('chat-update', async (mek) => {
		try {
			...
			const apiKey = `${spykey}` // mau apikey prem? hubungi wa.me/62895704368804
			...
			const ownerNumber = [`${nomorown}@s.whatsapp.net`] // replace this with your number
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'SpyBot'; if (!author) author = 'YusufRmdn';	
			...
			switch(command) {
				case 'help':
				case 'menu':
				tekss = `Menu Masih Kosong Gan!`
				yusufz.sendMessage(from, tekss, text, {quoted: mek})
				break
default:
